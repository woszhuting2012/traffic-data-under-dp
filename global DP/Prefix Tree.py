import sys
from step0_0_init import RETARDTIME,load_variable,save_variable,PTNode,PTree,block_x,block_y,START_TIME,END_TIME,UNIT_LEN,NET_NAME,TRA_NUM,EPSILON,TIMESTEP,MAX_TRA
...
for t in tqdm(range(START_TIME,END_TIME,TIMESTEP)):
    ...
    result = ptree_count(noisy_T,0,edge_id_list_partial,tree_edge_list,rest_random,t) #原始数据
    save_variable(result,'noisy_data/noisy_%d.pkl'%t)
    result = ptree_count(ptree_T,0,edge_id_list_partial,tree_edge_list,rest_random,t)
    save_variable(result,'ptree.pkl')
    error_origin = var_cal('noisy_data/noisy_%d.pkl'%t,'origin_data/origin_%d.pkl'%t)
    error_opt = var_cal('ptree.pkl','origin_data/origin_%d.pkl'%t)
    mse_org = np.var(error_origin)
    mse_opt = np.var(error_opt)
    org_sum_var += mse_org
    opt_sum_var += mse_opt

print('%.1f'%(org_sum_var/(END_TIME-START_TIME)*TIMESTEP))
print('%.1f'%(opt_sum_var/(END_TIME-START_TIME)*TIMESTEP))
end_runtime = time.time()
run_time = end_runtime - start_runtime
print(run_time)
